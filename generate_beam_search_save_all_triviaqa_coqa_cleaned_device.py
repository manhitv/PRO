...
if args.dataset == 'coqa':
    ...
    dataset = datasets.load_from_disk(f'{config.data_dir}/truthful_qa_{args.model}')
...
elif (args.dataset == 'trivia_qa') or (args.dataset == 'truthful_qa'):
    ...
    with torch.no_grad():
        ...
        for batch in tqdm.tqdm(dataloader):
            ...
            input_ids = torch.cat(batch['input_ids']).to(device).reshape(
                1, -1) if (args.dataset == 'trivia_qa') or (args.dataset == 'truthful_qa') else batch['input_ids'].to(device)
            ...
            input_length = input_ids.shape[1] if (args.dataset == 'trivia_qa') or (args.dataset == 'truthful_qa') else batch['input_ids'].shape[1]
            ...
            for i in range(generations.shape[0]):
                ...
                if args.dataset == 'coqa':
                    ...
                    few_shot_question = tokenizer.decode(input_ids[0])
                    question = few_shot_question.split('Q: ')[-1].split('A: ')[0]
                    sequence_dict = {
                        'prompt': input_ids[0],
                        'generations': generations[i],
