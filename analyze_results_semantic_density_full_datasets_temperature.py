...
# parser.add_argument('--correctness_threshold', type=float, default=0.3)
...
neg_log_likelihood_beam_search_all_list = []
...
for correctness_threshold in [0.3]:
        ...
        for run_id in run_ids_to_analyze:
            ...
            def get_generations_df():
                ...
                with open(f'{config.output_dir}/{model_name}_generations_all_{args.dataset}.pkl', 'rb') as infile:
                    ...
                    generations_df['correct'] = (generations_df['rougeL_beam_search_to_target_{}'.format(beam_id)] > correctness_threshold).astype('int')
            ...
            with open(f'{config.result_dir}/roc_curve_{model_name}_10sample_beam5.pkl', 'wb') as f:
            ...
            neg_log_likelihood_beam_search_list = []
            for index_tmp in result_df.index:
                neg_log_likelihood_beam_search_list.append(likelihoods_all[index_tmp]['neg_log_likelihood_of_beam_search_gen_{}'.format(beam_id)].cpu())
            ...
            neg_log_likelihood_beam_search_all_list+=neg_log_likelihood_beam_search_list
    ...
    result_dict['neg_log_likelihood_auroc'] = sklearn.metrics.roc_auc_score(1 - np.array(correct_all_list),
                                                            np.array(neg_log_likelihood_beam_search_all_list))
...
df = pd.DataFrame(list_df).rename(columns={
    ...
    'average_contradict_prob_auroc': 'Deg',
    'average_neg_log_likelihood_auroc': 'NL',
    'ln_predictive_entropy_auroc': 'NE',
